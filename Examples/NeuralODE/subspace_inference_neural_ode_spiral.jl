...
prob, n_ode =  BayesNeuralODE.generate_odes(:lv)
for i in 1:len	
	ode_data[:,i] = reshape(Array(solve(prob,Tsit5(),saveat=t))', :, 1)
end
ode_data_bkp = ode_data
ode_data += rand(Normal(0.0,0.03), 2*datasize,len)
(fig, f_axes) = PyPlot.subplots(ncols=1, nrows=1)

for i in 1:len
	f_axes.plot(t,vec(ode_data[1:datasize,i]), c="green", marker=".", label ="prediction")
end
...
@save "n_ode_weights_100_lv.bson" ps;

#Load trained parameters
@load "n_ode_weights_100_lv.bson" ps;
